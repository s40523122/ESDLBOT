...
void setup(){   
    ...
    if (c == 0x68) // WHO_AM_I should always be 0x68
        {    
            nh.loginfo("MPU6050 is online...");
            
            MPU6050SelfTest(SelfTest); // Start by performing self test and reporting values
            /*char Ax_trim[3]; char Ay_trim[3]; char Az_trim[3]; char Gx_trim[3]; char Gy_trim[3]; char Gz_trim[3];  
            char Ax_msg[50]; char Ay_msg[50]; char Az_msg[50]; char Gx_msg[50]; char Gy_msg[50]; char Gz_msg[50]; 
            
            dtostrf(SelfTest[0], 1, 2, Ax_trim);
            sprintf(Ax_msg, "x-axis self test: acceleration trim within : %s %% of factory value", Ax_trim);
            nh.loginfo(Ax_msg);
        
            dtostrf(SelfTest[1], 1, 2, Ay_trim);
            sprintf(Ay_msg, "y-axis self test: acceleration trim within : %s %% of factory value", Ay_trim);
            nh.loginfo(Ay_msg);
        
            dtostrf(SelfTest[2], 1, 2, Az_trim);
            sprintf(Az_msg, "z-axis self test: acceleration trim within : %s %% of factory value", Az_trim);
            nh.loginfo(Az_msg);
        
            dtostrf(SelfTest[3], 1, 2, Gx_trim);
            sprintf(Gx_msg, "x-axis self test: gyration trim within : %s %% of factory value", Gx_trim);
            nh.loginfo(Gx_msg);
            
            dtostrf(SelfTest[4], 1, 2, Gy_trim);
            sprintf(Gy_msg, "y-axis self test: gyration trim within : %s %% of factory value", Gy_trim);
            nh.loginfo(Gy_msg);
            
            dtostrf(SelfTest[5], 1, 2, Gz_trim);
            sprintf(Gz_msg, "z-axis self test: gyration trim within : %s %% of factory value", Gz_trim);
            nh.loginfo(Gz_msg);*/
            ...
            if(SelfTest[0] < 1.0f && SelfTest[1] < 1.0f && SelfTest[2] < 1.0f && SelfTest[3] < 1.0f && SelfTest[4] < 1.0f && SelfTest[5] < 1.0f) {
                ...
                initMPU6050(); 
                nh.loginfo("MPU6050 initialized for active data mode...."); // Initialize device for active mode read of acclerometer, gyroscope, and temperature
            }
        else{
            nh.loginfo("Could not connect to MPU6050: 0x");
  ...
  for(;;){
    ...
    if(readByte(MPU6050_ADDRESS, INT_STATUS) & 0x01) {  // check if data ready interrupt
      ...
      if(deltat > 5) {
          ...
          //Serial.print("X-acceleration: "); Serial.print(ax,2); Serial.print(" g "); 
          //Serial.print("Y-acceleration: "); Serial.print(ay,2); Serial.print(" g "); 
          //Serial.print("Z-acceleration: "); Serial.print(az,2); Serial.println(" g");
          imu_data.linear_acceleration.x = ax * 9.8;
          imu_data.linear_acceleration.y = ay * 9.8;
          imu_data.linear_acceleration.z = az * 9.8;
       
          // Print gyro values in degree/sec
          //Serial.print("X-gyro rate: "); Serial.print(gyrox*0.017453, 5); Serial.print(" radius/sec "); 
          //Serial.print("Y-gyro rate: "); Serial.print(gyroy*0.017453, 5); Serial.print(" radius/sec "); 
          //Serial.print("Z-gyro rate: "); Serial.print(gyroz*0.017453, 5); Serial.println(" radius/sec"); 
          imu_data.angular_velocity.x = gyrox * 0.017453;
          imu_data.angular_velocity.y = gyroy * 0.017453;
          imu_data.angular_velocity.z = gyroz * 0.017453;
